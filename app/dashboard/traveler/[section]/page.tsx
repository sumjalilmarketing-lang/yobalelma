...
export default async function TravelerSectionPage({
  params,
}: {
  params: Promise<{ section: string }>;
}) {
  const { section } = await params;
  const key = `traveler/${section}`;
  ...
  return <OperationalWorkspace config={config} returnTo={`/dashboard/traveler/${section}`} />;
}
