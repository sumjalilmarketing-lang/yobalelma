...
async function MissionList({ userId }: { userId: string }) {
  ...
  if (!data?.length) {
    return <EmptyState title="Aucune mission" description="Les missions apparaissent ici apres dispatch automatique d'une expedition nationale." />;
  }
    ...
    <DataGrid>
      {data.map((mission) => {
        ...
        return (
          <DataCard
            key={mission.id}
            title={shipment?.tracking_code ?? mission.shipment_id}
