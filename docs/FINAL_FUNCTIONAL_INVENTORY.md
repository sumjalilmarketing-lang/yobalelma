...
| Client tracking | client | `/dashboard/client/tracking` | `shipments`, `shipment_status_events`, `delivery_proofs` | Pilotable workspace | E2E access | Real counters | Public tracking missing |
...
| Dispatch engine | operations_manager | migration | `dispatch_jobs`, `dispatch_candidates`, `dispatch_events` | Local schema ready | SQL review | Real schema pending push | CLI DB blocked |
...
| QR handover/destination | traveler/hub/relay | `/api/qr/*` | `handover_qr_tokens` | Functional foundation | E2E | Real RPC/API | QR image rendering pending |
