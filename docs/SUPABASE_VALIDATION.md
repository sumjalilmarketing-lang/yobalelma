...
| --- | --- |
| Variables secretes presentes dans shell Codex | Non |
| URL projet verifiee depuis `.env.local` | Oui |
...
`SUPABASE_ACCESS_TOKEN` permet a `npm run validate:supabase` de recuperer les API keys via Management API si les cles projet ne sont pas injectees directement. `DATABASE_URL` ou `POSTGRES_URL` reste necessaire pour appliquer les migrations sans saisie interactive du mot de passe Postgres.
