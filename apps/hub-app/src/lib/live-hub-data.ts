...
export async function loadLiveHubState(session: HubSession): Promise<HubState | null> {
  ...
  try {
    ...
    const profileKyc = new Map(profileRows.map((row) => [
      text(row, "id"),
      oneOf(text(row, "kyc_status"), ["verified", "pending", "rejected"] as const, "pending"),
    ]));
