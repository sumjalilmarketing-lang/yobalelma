...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: decision, error } = await callSupabaseRpc<string>(configuredClient, "record_relay_package_control", {
      p_height_cm: control.dimensions.heightCm,
      p_idempotency_key: control.idempotencyKey,
      p_length_cm: control.dimensions.lengthCm,
      p_quality_score: control.qualityScore,
      p_relay_point_id: control.relayPointId,
      p_tracking_code: control.trackingCode,
      p_weight_kg: control.weightKg,
      p_width_cm: control.dimensions.widthCm,
    });
