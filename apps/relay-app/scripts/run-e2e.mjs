...
try {
  if (process.env.PLAYWRIGHT_SKIP_WEBSERVER !== "1" && !(await ready())) {
    server = spawn(process.execPath, [nextCli, "dev", "--hostname", "127.0.0.1", "--port", String(port)], {
      cwd: appDir, env: { ...process.env, RELAY_ENABLE_DEMO_AUTH: "1", NEXT_DIST_DIR: ".next-e2e", PORT: String(port) }, stdio: "inherit", windowsHide: true,
    });
  ...
  const child = spawn(process.execPath, [playwrightCli, "test", ...process.argv.slice(2)], {
    cwd: appDir, env: { ...process.env, RELAY_ENABLE_DEMO_AUTH: "1", PLAYWRIGHT_BASE_URL: baseURL, PLAYWRIGHT_SKIP_WEBSERVER: "1" }, stdio: "inherit", windowsHide: true,
  });
