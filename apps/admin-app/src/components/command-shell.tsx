...
export function CommandShell({ children, directions, session }: { children: React.ReactNode; directions: GovernanceDirection[]; session: AdminSession }) {
  ...
  return <div className="min-h-screen bg-background">
    <header className="fixed inset-x-0 top-0 z-50 border-b bg-background/90 backdrop-blur-xl"><div className="mx-auto flex h-16 max-w-[1800px] items-center gap-3 px-3 md:px-5">
    ...
    <div className="mx-auto grid max-w-[1800px] gap-5 px-3 pb-24 pt-20 md:px-5 lg:grid-cols-[300px_1fr] lg:pb-8">
      <aside className="hidden lg:block">
        <div className="sticky top-20 max-h-[calc(100vh-6rem)] overflow-y-auto rounded-xl border bg-background p-3">
...
function navClass(pathname: string, href: string) {
  ...
  return cn("flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-bold transition hover:bg-primary hover:text-white", active ? "bg-black text-white" : "text-muted-foreground");
}
