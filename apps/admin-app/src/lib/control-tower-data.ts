...
export type TowerView="supervision"|"dispatch"|"executive"|"finance"|"support"|"partner"|"customs"|"operations";
export function allowedTowerViews(session:AdminSession):TowerView[]{const roles=session.roleIds;if(roles.some((r)=>["super_admin","admin","country_manager","auditor"].includes(r)))return ["supervision","dispatch","executive","finance","support","partner","customs","operations"];if(roles.some((r)=>r.startsWith("finance")||["accounting_agent","payment_agent","reconciliation_agent"].includes(r)))return ["finance"];if(roles.some((r)=>r.includes("customs")||r.includes("compliance")))return ["customs"];if(roles.some((r)=>r.includes("support")))return ["support"];if(roles.some((r)=>r.includes("partner")||r.includes("orange")))return ["partner"];if(roles.some((r)=>r.includes("dispatch")))return ["dispatch","operations","supervision"];return ["supervision","operations"];}
