...
const driver=client(); const manager=client(); const supervisor=client(); const denied=client(); const anonymous=client();
await signIn(driver,"pilot.collection-driver@yobalelma.test"); await signIn(manager,"pilot.collection-manager@yobalelma.test"); await signIn(supervisor,"pilot.collection-supervisor@yobalelma.test"); await signIn(denied,"pilot.collection-denied@yobalelma.test");
const driverUser=(await driver.auth.getUser()).data.user; const managerUser=(await manager.auth.getUser()).data.user; const supervisorUser=(await supervisor.auth.getUser()).data.user; if(!driverUser||!managerUser||!supervisorUser)throw new Error("Pilot identities unavailable.");
...
const supervisorRoutes=await supervisor.from("collection_routes").select("id"); assert(!supervisorRoutes.error&&(supervisorRoutes.data?.length??0)>=1,"operations manager sees collection routes");
...
console.log(JSON.stringify({ok:true,checks:["real auth roles","driver route isolation","manager visibility","operations manager visibility","client and anonymous denial","GPS RPC ownership","direct assignment denial","offline idempotency","manager conflict resolution"],routeId}));
