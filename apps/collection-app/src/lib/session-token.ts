...
export const collectionSessionCookie = "yb_collection_session";
...
function secret() {
  const value = process.env.COLLECTION_SESSION_SECRET?.trim();
  if (value) return value;
  return process.env.NODE_ENV === "production" ? null : "local-yobalelma-collection-session";
}
...
export async function createCollectionSessionToken(session: CollectionSession) {
  const configured = secret(); if (!configured) throw new Error("Collection session secret is required in production.");
...
export async function verifyCollectionSessionToken(token?: string | null): Promise<CollectionSession | null> {
  ...
  try {
    ...
    const session = JSON.parse(new TextDecoder().decode(decode(payload))) as CollectionSession;
