...
export async function POST(request: NextRequest) {
  try {
    ...
    const returnTo = requested.startsWith("/") && !requested.startsWith("//") && !requested.includes("\\") ? requested : "/collection";
  ...
  } catch { const url = new URL("/auth/sign-in", requestOrigin(request)); url.searchParams.set("error", "Les informations de connexion sont incorrectes ou le service est momentanément indisponible."); return NextResponse.redirect(url, { status: 303 }); }
}
