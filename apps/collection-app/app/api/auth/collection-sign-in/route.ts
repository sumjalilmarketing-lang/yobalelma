...
export async function POST(request: NextRequest) {
  try {
    ...
    if (process.env.NODE_ENV === "production" && process.env.COLLECTION_ENABLE_DEMO_AUTH !== "1") throw new Error("Mode démonstration désactivé.");
...
function safePath(value: FormDataEntryValue | null) { return typeof value === "string" && value.startsWith("/") && !value.startsWith("//") ? value : "/collection"; }
