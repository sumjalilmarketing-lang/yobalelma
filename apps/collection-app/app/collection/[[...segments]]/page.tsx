...
import { loadCollectionState } from "@collection-app/src/lib/collection-data";
export default async function CollectionPage({ params }: { params: Promise<{ segments?: string[] }> }) {
  ...
  const session = await requireCollectionSession(pathname); const state = await loadCollectionState(session);
