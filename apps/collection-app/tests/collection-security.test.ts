...
describe("Collection access control", () => {
  ...
  it("persists GPS through the consent-controlled batch function", async () => { const source=await readFile(path.resolve(process.cwd(),"apps/collection-app/app/api/collection/gps/route.ts"),"utf8"); const consent=await readFile(path.resolve(process.cwd(),"apps/collection-app/app/api/collection/location-consent/route.ts"),"utf8"); expect(source).toContain('rpc("record_operational_positions_batch"'); expect(consent).toContain('rpc("set_operational_location_consent"'); expect(source).toContain('session.source !== "supabase"'); expect(source).toContain("positions:z.array(positionSchema).min(1).max(50)"); });
});
