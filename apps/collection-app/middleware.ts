...
import { isCollectionRole, type CollectionRole } from "./src/lib/types";

export async function middleware(request: NextRequest) {
  const pathname = request.nextUrl.pathname;
  const requestId = correlationId(request);
  const isPage = pathname.startsWith("/collection");
  const isApi = pathname.startsWith("/api/collection");
  ...
  if (!canUseCollectionRoute(role, pathname, request.method)) {
    return isApi
      ? correlated(NextResponse.json({ error: "Accès refusé." }, { status: 403 }), requestId)
      : correlated(NextResponse.redirect(new URL("/collection", requestOrigin(request))), requestId);
  }
  structuredLog("info", "collection_request", { method: request.method, pathname, requestId, role });
  const headers = new Headers(request.headers);
  headers.set("x-correlation-id", requestId);
  return correlated(supabaseAccess?.response ?? NextResponse.next({ request: { headers } }), requestId);
}

async function resolveSupabaseAccess(request: NextRequest): Promise<{ response: NextResponse; role: CollectionRole } | null> {
  ...
  const role = [...(rolesResult.data?.map((item) => item.role_id) ?? []), profileResult.data?.primary_role, profileResult.data?.role].find(isCollectionRole);
