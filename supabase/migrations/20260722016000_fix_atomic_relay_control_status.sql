...
begin
  ...
  if p_weight_kg <= 0 or p_weight_kg > 300
    ...
    or p_quality_score not between 1 and 100 then raise exception 'Invalid package control'; end if;
  ...
  update public.relay_inventory set
    ...
    status = (case when v_decision = 'accepted' then 'stored' else 'exception' end)::public.relay_inventory_status,
